...
window.addEventListener('load',function(){
     ...
     function renderDom(data) {
        ...
        for (let i = 0; i < data.data.list.length; i++) {
            ...
            <ul>
                ...
                <li>招募开始时间:${new Date(data.data.list[i].begin).format('y年M月d日')}</li>
                <li>招募结束时间:${new Date(data.data.list[i].end).format('y年M月d日')}</li>
    ...
    expand_peo.addEventListener('click', function (e) {
        if (e.target.classList.contains('peo_d')) {
            e.target.index == 0 ? null : expand_opt.data.numberScope = e.target.index;
        }
        console.log('人数范围区间为：' + expand_opt.data.numberScope);
    })

    // 招募状态
    expand_situ.addEventListener('click',function(){
        if(e.target.classList.contains('situation')){
            e.target.index?expand_opt.data.status=e.target.index:null;
        }
    })
